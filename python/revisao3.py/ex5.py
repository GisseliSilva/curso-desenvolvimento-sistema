'''5.	Crie uma classe Elevador com atributos para andar atual, total de andares e capacidade.
Implemente métodos para subir e descer andares.'''

class Elevador:
    def __init__(self, capacidade,total_andrades):
        self.capacidade = capacidade
        self.total_andares = total_andrades
        self.andar_atual = 0 

    
    def subir(self):
        if self.andar_atual < self.total_andrades:
            self.andar_atual +=1
    ...
    def descer(self):
        if self.andar_atual < 0:
            self.andar_atual -=1
            print(f"Descer para o andar {self.andar_atual}")
        else:
